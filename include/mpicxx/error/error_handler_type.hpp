...
namespace mpicxx {
    ...
    inline std::ostream& operator<<(std::ostream& out, const error_handler_type eht) {
        ...
        std::size_t idx = 0;
        if (detail::is_type_set(eht, error_handler_type::comm)) {
            strs[idx++] = "COMM";
        }
        if (detail::is_type_set(eht, error_handler_type::file)) {
            strs[idx++] = "FILE";
        }
        if (detail::is_type_set(eht, error_handler_type::win)) {
            strs[idx++] = "WIN";
        }
    ...
    inline error_handler_type error_handler_type_from_string(const std::string_view sv) {
        ...
        do {
            ...
            if (substr.compare("COMM") == 0) {
                eht |= error_handler_type::comm;
            } else if (substr.compare("FILE") == 0) {
                eht |= error_handler_type::file;
            } else if (substr.compare("WIN") == 0) {
                eht |= error_handler_type::win;
            }
